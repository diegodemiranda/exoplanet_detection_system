...
try:
    from backend.data_ingestion import build_dataset_from_nasa
...
def train_and_save(mode: str,
    ...
    if mode == "synthetic":
        ...
        if build_dataset_from_nasa is None:
            raise RuntimeError("data_ingestion/build_dataset_from_nasa not available in the environment.")
    ...
    try:
        ...
        print(f"WARN: Validation evaluation failed: {e}")
        report = None
        val_accuracy = None
    ...
    try:
        ...
        print(f"WARN: load_model smoke test failed: {e}")
...
def parse_args():
    p = argparse.ArgumentParser(description="ExoplanetDetector training")
    p.add_argument("--mode", choices=["synthetic", "nasa"], default="synthetic",
                   help="Data source: fast synthetic or NASA (slow)")
    ...
    p.add_argument("--limit-per-class", type=int, default=200,
                   help="NASA mode only: max per class (total)")
    p.add_argument("--missions", nargs="*", default=["Kepler", "TESS", "K2"],
                   help="NASA mode only: missions to include")
    p.add_argument("--cap-per-mission-per-class", type=int, default=None,
                   help="NASA mode only: per-mission-per-class cap to balance catalogs")
    return p.parse_args()
