def preprocess_kepler_data(light_curve_file):
    """
    Full preprocessing pipeline for Kepler/TESS data
    """
    import numpy as np
    ...
    # 1. Load data
    flux = light_curve_file['PDCSAP_FLUX']  # Flux corrected by PDC
    time = light_curve_file['TIME']
    quality = light_curve_file['SAP_QUALITY']

    # 2. Remove bad quality points
    ...
    # 4. Detrending using Savitzky-Golay filter
    if len(flux_valid) > 99:  # Required for window_length
    ...
    # 5. Robust normalization (median + MAD)
    ...
    # 7. Interpolation to uniform grid
    if len(time_valid) > 1:
        ...
        time_uniform = np.linspace(0, 1, 2001)
