...
from backend.services import OptimizedExoplanetDetectorService
...
from backend.data_ingestion import fetch_catalogs, _load_flux_from_local_cache, _download_light_curve
...
async def detection_exception_handler(request, exc: ExoplanetDetectionError):
    ...
    logger.error(f"Detection error: {exc.message} - {exc.details}")
    return JSONResponse(
        status_code=400,
        content={
...
async def root():
    ...
    if index_path.exists():
        ...
        return HTMLResponse(
            ...
            <html>
                <head>
                    <title>Exoplanet Detector API</title>
                    <style>
                        body {
                ...
                <body>
                    <h1>🔭 Exoplanet Detector API</h1>
                    <p>React frontend not built yet. Run:</p>
...
if frontend_dist.exists():
    ...
    logger.warning("⚠️  React frontend not found. Run 'cd frontend && npm run build'")
